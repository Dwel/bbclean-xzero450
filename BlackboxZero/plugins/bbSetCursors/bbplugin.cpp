...
#include <string>
#include <algorithm>
#include "Console\ConsoleAdapter.h"
...
LPCSTR szVersion        = "bbSetCursors 0.0.2 (BBClean SVN)";
LPCSTR szAppName        = "bbSetCursors";
LPCSTR szInfoVersion    = "0.0.2 (BBClean SVN)";
...
void about_box(void)
{
	char szTemp[1000];
	sprintf(szTemp,
		"%s - Cursor theme extension for bb4win styles."
		"\n"
		"\n© 2004 %s"
		"\n%s"
...
void UpdateStyleInfo()
{
	...
	try {
		if (readFileInfo(path))
			applyCursors();
	} catch(...) {
		MessageBox(0,"The style file did not load properly!\nPlease notify Noccy about this at the LostInTheBox forums!",
		szVersion,MB_OK | MB_ICONERROR | MB_TOPMOST);
	}
}
