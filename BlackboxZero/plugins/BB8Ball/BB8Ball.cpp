...
int beginPlugin(HINSTANCE hPluginInstance)
{
    ...
    MakeSticky(hwndBB8Ball);

    if(alwaysOnTop) {
        SetWindowPos(hwndBB8Ball, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOMOVE);
    }
...
void ReadRCSettings() {
    ...
    char defaultpath[MAX_LINE_LENGTH];
    int nLen;
    ...
    if(ReadString(rcpath, "BB8Ball.inSlit:", NULL) == NULL) {
        wantInSlit = true;
    } else {
        wantInSlit = ReadBool(rcpath, "BB8Ball.inSlit:", true);
    }
...
void applyTransparencySettings() {
    ...
    if(inSlit && (transparency || fullTrans)) {
        SetWindowLong(hwndBB8Ball, GWL_EXSTYLE, WS_EX_TOOLWINDOW);    
        return;
    }

    if(transparency || fullTrans) {
    ...
    COLORREF colorRef = fullTrans ? 0x202020 : NULL;
    DWORD dwFlags = (fullTrans ? LWA_COLORKEY : 0) | (transparency ? LWA_ALPHA : 0);

    BBSetLayeredWindowAttributes(hwndBB8Ball, colorRef, (BYTE)alpha, dwFlags);
}
