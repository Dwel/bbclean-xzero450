...
BBLIB_EXPORT void parse_item (LPTSTR szItem, StyleItem * item);
BBLIB_EXPORT int findtex (TCHAR const * p, int prop);
struct styleprop
{
    TCHAR const * key;
    int val;
};
BBLIB_EXPORT styleprop const * get_styleprop (int prop);
BBLIB_EXPORT int find_in_propitem (styleprop const * props, int value);
